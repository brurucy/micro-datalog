...
fn modify_original_rule(rule: &Rule, adorned_head: &AdornedAtom) -> Rule {
    ...
    // Use existing function to get bound vars
    let bound_vars = get_bound_vars_from_adorned(&adorned_head);

    // Get bound terms from the head of the original rule based on adornment pattern
    ...
    new_body.push(magic_pred);

    // Add body predicates with proper adornments
    for (pos, body_atom) in rule.body.iter().enumerate() {
        if is_derived_predicate(rule, &body_atom.symbol) {
            // Compute bound variables at this position
            let curr_bound_vars = compute_bound_vars_at_position(rule, pos, &bound_vars);
            // Create adorned version for derived predicates
            let adorned_body = AdornedAtom::from_atom_and_bound_vars(body_atom, &curr_bound_vars);
...
mod tests {
    ...
    fn test_modify_original_rule() {
        // Original rule: p(X,Y) :- q(X,Z), p(Z,Y)
    ...
    fn test_modify_original_rule_with_multiple_bound() {
        // Original rule: p(X,Y,Z) :- q(X,Y), r(Y,Z)
